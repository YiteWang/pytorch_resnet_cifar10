...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='B',track_relu=False):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2, track_relu = True)
        self.linear = nn.Linear(64, num_classes)

        self.apply(_weights_init)

    def _make_layer(self, block, planes, num_blocks, stride, track_relu = False):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for index,stride in enumerate(strides):
            if index == len(strides)-1:
                layers.append(block(self.in_planes, planes, stride, track_relu=track_relu))
            else:
...
def resnet20(ONI=False, cut=False, T_iter=5):
    if ONI:
        return ResNet_NC(BasicBlock_NC, [3, 3, 3], T_iter=T_iter)
    else:
        if cut:
            block = BasicBlockwoskip
        else:
            block = BasicBlock
